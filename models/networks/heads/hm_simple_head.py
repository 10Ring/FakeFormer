...
class TopdownHeatmapSimpleHead(TopdownHeatmapBaseHead):
            ...
            for head, out_channel in self.heads.items():
                if head == 'hm':
                    layer = nn.Conv2d(
                                ...
                                padding=padding)
                    layers.append(layer)
                    # final_layer[head] = nn.DataParallel(nn.Sequential(*layers), device_ids=devices).cuda().to(torch.float64)
                    final_layer[head] = nn.Sequential(*layers)
                elif head == 'cls':
                    layer = nn.Sequential(
                                nn.Flatten(),
    ...
    def _transform_inputs(self, inputs):
        ...
        """
        inputs_cls = None
        inputs_spatial_cls_token = None
        if isinstance(inputs, dict):
            assert 'embed' in inputs.keys(), "Embed token must be present in the input dict key"
            inputs_cls = inputs['cls'] if 'cls' in inputs.keys() else None
            inputs_spatial_cls_token = inputs['spatial_cls_token'] if 'spatial_cls_token' in inputs.keys() else None
            inputs = inputs['embed']
        ...
        if not isinstance(inputs, list):
            ...
            return inputs, inputs_cls, inputs_spatial_cls_token
    ...
    def forward(self, x):
        ...
        """
        x_embed, x_cls, x_spatial_cls_token = self._transform_inputs(x)
        if x_embed.dim() > 4:
            B, T, L, H, W = x_embed.shape
            x_embed = torch.mean(x_embed, dim=1, keepdim=False)

        x = self.deconv_layers(x_embed)

        x_outs = {}
        # for head in self.heads.keys():
        if x_cls is not None:
            assert hasattr(self, 'cls'), "There must be a Classification Head, please check the head design!"
            x_outs["cls"] = self.__getattr__('cls')(x_cls)

        # for head in self.heads.keys():
        if x_spatial_cls_token is not None:
            assert hasattr(self, 'temp_loc'), "There must be a head for temporal localization, please check the head design!"
            x_outs["temp_loc"] = self.__getattr__('temp_loc')(x_spatial_cls_token)
            
        if 'hm' in self.heads.keys():
            assert hasattr(self, 'hm'), "There must always be a Heatmap Head in the head!"
            x_outs["hm"] = self.__getattr__('hm')(x)
        return [x_outs]
