...
class Block2D(nn.Module):
    ...
    def forward(self, x, B, T, W):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            spatial_cls_token = torch.mean(cls_token, 2, False) ## averaging for every patch, use for temporal localization
            ...
            # spatial_cls_token = torch.mean(rearrange(x[:, 1:], 'b (h w t) m -> b t (h w m)', b=B, h=H, w=W, t=T), 2, False)
            return x, spatial_cls_token
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x, spatial_cls_token = blk(x, B, T, Wp)
        ...
        if self.attention_type != 'space_only':
            xp = rearrange(xp, 'b (n t) h w -> b t n h w', b=B, t=T, h=Hp, w=Wp)
        res["embed"] = xp

        if spatial_cls_token is not None:
            res["spatial_cls_token"] = spatial_cls_token
        else:
            res["spatial_cls_token"] = torch.mean(rearrange(xp, 'b t n h w -> b t (n h w)', b=B, t=T, h=Hp, w=Wp), 2, False)
