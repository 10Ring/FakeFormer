...
def _neg_loss(pred, gt, epsilon=0.1, noise_distribution=0.2, alpha=0.25):
...
class CombinedFocalLoss(BaseLoss):
    ...
    def forward(self, 
        ...
        loss_hm = self.hm_loss(hm_outputs_, hm_targets, alpha=self.hm_lmda)
...
class CombinedMSELoss(BaseLoss):
    ...
    def _temp_loc_loss(self, preds, gts):
        """
    ...
    def forward(self, 
                ...
                temp_loc_gts=None):
        loss_return = {}
        if not hm_use_cst_loss:
            loss_hm = self.criterion_hm(hm_outputs, hm_targets, target_weight=target_weight)
        else:
            loss_hm = self._consistency_loss(hm_outputs, hm_targets)
        loss_return['hm'] = loss_hm

        loss_cls = self._cls_loss(cls_preds, cls_gts)
