...
def gen_SBI(img, landmark, index=None, debug=False, dynamic_blending_prob=False):
    ...
    # Getting ConvexHull
    mask = random_get_hull(landmark, img)
    ...
    # if not distort:
    source, mask = randaffine(source, mask[:,:,0])
    # else:
    ...
    img_blended, mask, blending_prob = dynamic_blend(source, img, mask, dynamic_blending_prob=dynamic_blending_prob)
    img_blended = img_blended.astype(np.uint8)
    img = img.astype(np.uint8)

    return img, img_blended, mask, blending_prob


def gen_target(background_face, background_landmark, margin=[20, 20], index=None, debug=False, dynamic_blending_prob=False):
    if isinstance(background_face, str):
        background_face = load_image(background_face)
    
    margin_x, margin_y = margin

    background_face, face_img, mask_f, blending_prob = gen_SBI(background_face, background_landmark, index=index, debug=debug, \
                                                               dynamic_blending_prob=dynamic_blending_prob)
    ...
    fake_intensity = blending_prob*(1-blending_prob)* 4
