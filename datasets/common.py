...
class CommonDataset(Dataset, ABC):
    ...
    def _gen_vul_parts(self, blending_mask):
        Hp, Wp = int(self._cfg.HEATMAP_SIZE[0]), int(self._cfg.HEATMAP_SIZE[1])
        H, W, C = blending_mask.shape

        assert (H // Hp) == (W // Wp)
        vul_parts = np.zeros((H // Hp, W // Wp))

        for i in range(0, H // Hp):
            for j in range(0, W // Wp):
                blending_part = blending_mask[(Hp*i):(Hp*(i+1)), (Wp*j):(Wp*(j+1)), 0]
                part_intensity = np.mean(blending_part)
                vul_parts[i, j] = part_intensity
        vul_parts = np.expand_dims(vul_parts, -1)

        return vul_parts

    def _encode_target(self, target_mask, normalize_max=False, fake_intensity=0.5):
        ...
        # lower_bound_intensity = max(0.0, (fake_intensity-0.05))
        # upper_bound_intensity = min(1.0, (fake_intensity+0.05))
    ...
    def _new_encode_target(self, target_mask, normalize_max=True, fake_intensity=0.5):
        assert self.heatmap_type == 'gaussian', 'Only Gaussian Heatmap is supported now!'
    ...
    def _encode_target_v2(self, target_mask, normalize_max=True, fake_intensity=0.5):
        assert self.heatmap_type == 'gaussian', 'Only Gaussian Heatmap is supported now!'
        n_outputs = 1
        hm_w = self._cfg.HEATMAP_SIZE[1]
        hm_h = self._cfg.HEATMAP_SIZE[0]
        ...
        if self.data_type == "image":
            ...
            if self.train:
                ...
                for vid_id in vid_dict.keys():
                    ...
                    for k in range(0, vid_len, n_fs):
                        try:
                            if (k + n_fs) <= vid_len:
                                new_vid_id = '_'.join([vid_id, str(k)])
