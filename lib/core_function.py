...
def get_batch_data(batch_data):
    ...
    return inputs, labels, targets, heatmaps, cstency_heatmaps, offsets, temp_locs
...
def train(cfg, model, critetion, optimizer, epoch, data_loader, logger, writer, devices, trainIters, metrics_base='combine'):
    ...
    for i, batch_data in enumerate(data_loader):
        inputs, labels, targets, heatmaps, cstency_heatmaps, offsets, temp_locs = get_batch_data(batch_data)
        ...
        for idx in loop:
            ...
            if 'Combined' in cfg.TRAIN.loss.type:
                ...
                if temp_locs is not None:
                    temp_locs = temp_locs.cuda().to(non_blocking=True, dtype=torch.float64)
                ...
                loss = critetion(outputs_cls.sigmoid(), labels)
