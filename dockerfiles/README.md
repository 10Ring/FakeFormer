...
4. Check the status of the image created:
   ...
   2. You should see something similiar:
        ...
        |----------|---------|-------|------|---|
        |fakeformer| efd422370750|12 minutes ago| 18.4GB |latest |
5. Run a container from the created image:
   1. Run command
        ```shell
        docker run -v <data_path>:<path_in_container> --gpus 'all,capabilities=utility' -it fakeformer /bin/bash
        ```
   2. Check the container created:
       ...
       2. Check the result:
            ...
            |-----------|-----|-------|-------|------|-----|-----|
            |0203c192febb|fakeformer|"/bin/bash"| 29 seconds ago |Up 28 seconds | |determined_cannon|
...
6. Inside the docker container, you can clone or mount the repository from outside:
    ```shell
    cd /workspace/
    git clone https://github.com/XXX/FakeFormer.git
    cd FakeFormer/
    ```
