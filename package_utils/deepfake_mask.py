...
def dynamic_blend(source,target,mask, dynamic_blending_prob=False):
    blending_prob = 0.5 # The max distribution of BG used for clipping values in blending masks
    mask_blured = get_blend_mask(mask)
    blend_list = [0.25,0.5,0.75,1,1,1]
    blend_ratio = blend_list[np.random.randint(len(blend_list))]
    mask_blured_ = mask_blured * blend_ratio

    # Applying dynamic blending probs
    if dynamic_blending_prob:
        if random.random() < 0.5:
            blending_prob = np.random.uniform(0.5, 0.99)
            # mask_blured_[mask_blured_ < blending_prob] = 0.0 # Removing remaining parts

    img_blended = (mask_blured_ * source + (1 - mask_blured_) * target)
    return img_blended, mask_blured, blending_prob
